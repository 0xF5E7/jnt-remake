...
public class VirtualMachine implements Opcodes {
    ...
    private final int selectedPushOpcode;
    ...
    public VirtualMachine(VirtualMachineInterface vmi, byte[] operands, int selectedPushOpcode) {
        this.vmi                = vmi;
        this.operands           = operands;
        this.selectedPushOpcode = selectedPushOpcode;
    ...
    public InsnList generate() {
        ...
        list.add(new LookupSwitchInsnNode(
                push32Range.getStart(),          // default → PUSH32
                new int[]{PUSH8, PUSH16, PUSH32},
                new LabelNode[]{
                        push8Range.getStart(),
        ...
        list.add(new IntInsnNode(BIPUSH, operands[3]));   // low byte of value
        list.add(new InsnNode(IASTORE));
        list.add(new JumpInsnNode(GOTO, rangeEnd));
        list.add(push8Range.getEnd());
        int m16 = ((operands[2] & 0xFF) << 8) | (operands[3] & 0xFF);
        list.add(push16Range.getStart());
        list.add(new VarInsnNode(ALOAD, vmi.stackLocal()));
        list.add(new IincInsnNode(vmi.pointerLocal(), 1));
        list.add(new VarInsnNode(ILOAD, vmi.pointerLocal()));
        list.add(new IntInsnNode(SIPUSH, (short) m16));
        list.add(new InsnNode(IASTORE));
        list.add(new JumpInsnNode(GOTO, rangeEnd));
        list.add(push16Range.getEnd());

        // ── PUSH32 handler: store full 32-bit value ───────────────────────────
