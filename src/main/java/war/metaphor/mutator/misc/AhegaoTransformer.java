...
public class AhegaoTransformer extends Mutator {
    ...
    private void injectIntoClass(JClassNode classNode) {
        ...
        while (classNode.fields.stream().anyMatch(f -> ((FieldNode) f).name.equals(fieldName + ""))) {
            fieldName += (char) ThreadLocalRandom.current().nextInt('a', 'z' + 1);
        }
        ...
        //   nothing_to_see_here = new String[array.length];
    ...
    /** Returns a field name that is not already used in this class. */
    private static String resolveUniqueName(JClassNode classNode, String base) {
        String name = base;
        while (classNode.fields.stream().anyMatch(f -> ((FieldNode) f).name.equals(name))) {
